...
function findtheBigOne()
  ...
  for i=1,3 do
    for j=1,5 do
      a=getColor(ehl[i][j].x-15,ehl[i][j].y-64)
...
function deployTroop( u )
  ...
  for i=1,#(u) do
    ...
    while pg<=u[i].page do
      preciseDrag(unitx,unity[11]-2,unitx,unity[1]+2,20)
      mSleep(200)
      pg=pg+1
    end
...
function weaponCoordinate( wpn, armrack, ammorack )
  local c
  c = 850 - 65*wpn - 48*(armrack-1) + (armrack-ammorack)*20
  return 742,c
end
...
---          dragDrop(ehl[2][3].x, ehl[2][3].y, xx,yy, 5)      -- indirect aiming scope drag
---          dragDrop(ehl[1][3].x, ehl[1][3].y, xx,yy, 5)      -- indirect aiming scope drag

        else
          click( ehl[xi][yi].x, ehl[xi][yi].y )   -- touch enemy block directly(no aiming scope)
---          click( xx,yy )   -- touch enemy block directly(no aim scope)
          touchScreen(382,956)
        end
      end
    end
...
function reEnterBN()
  ...
  for i=1,5 do      -- Try 5 times
    ...
    local found = switchMapWithPop( MM_base, promo_popups, 3, 70 )             -- wait for the base map mark: worldmap button
