function waitForImage( imgFile,d,x1,y1,x2,y2,color,seconds )
	...
	repeat
		mSleep(1000)
		rx,ry=findImageInRegionFuzzy(imgFile,d,x1,y1,x2,y2,color)
       	waitfor=waitfor+1
        if waitfor == seconds then
             rx=1
             ry=1
        end
	until  rx~=-1 and ry~=-1
...
waitForImage("abort_v.png",90,274,1815,368,2047,0,10)  --wait the abort button for 10 seconds
--mSleep(2851)
...
touchDown(1,1484,1223) --15 select yale
mSleep(49)
touchUp(1,1484,1223)
mSleep(184)
...
waitForImage("smallhouse_v.png",90,171,1881,304,2047,0,35)
