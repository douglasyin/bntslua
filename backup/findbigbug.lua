...
function getBigBug( inaccuracy )
  ...
  for i=1,2 do
    for j=1,5 do
      ...
      dialog(string.format("%d,%d,%0x",i,j,aa),3)
      -- aa=getColorRGB(ehl[i][j].x+51,ehl[i][j].y-99)
      if aa<0x880000 and aa>0x800000 then 
      -- if aa<180 then
        keepScreen(false)
        return i,j,true
      end
  ...
  for j=2,4 do
    ...
    -- if rx ~= -1 and ry ~= -1 then
    aa=getColor(ehl[i][j].x+51,ehl[i][j].y-99)
    dialog(string.format("%d,%d,0x%0x",i,j,aa),3)
    -- aa=getColorRGB(ehl[i][j].x+51,ehl[i][j].y-99)
    if aa<0x880000 and aa>0x800000 then 
    -- if aa<180 then
      keepScreen(false)
...
function bossStrike(count)
  ...
  for i=1,count do
    ...
    -- click(BS_OY.x,BS_OY.y)      -- kick off Oh yeah
    ...
    lx,ly=findColorInRegionFuzzy(BS13_clgicon.color, 90, BS13_clgicon.x,BS13_clgicon.y,BS13_clgicon.x,BS13_clgicon.y)
    ...
    if found==false then
      found = waitMultiPixel( MM_BS13_ad, 90, 3 )                       -- Limit time offer AD
      if found==true then
...
row,col,found=getBigBug(85)

if found==true then
  dialog("Found  "..string.format("%d,%d",row,col), 5)
end
-- choice = dialogRet("Chose your Mission","Cancel","BS","Bigfoot",10)
