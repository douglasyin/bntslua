...
function deployTroop( u )
  ...
  for i=1,#(u) do
    click(1375, unitselecty)                -- unit category button
...
function clearLastBattle()
  ...
  x,y=findImageInRegionFuzzy("savedbattle_v.png",90,0,0,700,300,0)     -- The red X
...
while success==true do
  ...
  if fine==false then     -- failed fighting on bigfoot land
    ...
    for i=1,3 do            -- try relaunch BN and go to bigfoot's home for 3 times
      ...
      found = waitMultiPixel( MM_worldMap, 90, 25 )                          -- wait for the world map mark: pvp button
