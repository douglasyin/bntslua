...
function bossStrike(count)
  ...
  for i=1,count do
    ...
    rx = waitMultiPixel( BM_beginGrey_BS, 95, 30 )           --Wait the round grey button for 30 seconds
    if rx == false then
      return false,i
    end
    ...
    rounds,finished,bigone = proceedAttackSequence( "boss", "BS" )     -- Proceed the attack plan
    ...
    found = waitMultiPixel( BS_BSFT, 90, 35 )                      -- wait for the map mark: battle button

    if found==true then
      now=os.time()
      bt=now-b_start
      tt=now-t_start
      appendLog(LOGFILE, "[BS]"..string.format(" battle %d, %d rounds, %d sec, Totally %d sec, %4.1f sec/battle", 
                                                tic, rounds, bt, tt, tt/tic ) )
    else
      return false,i      -- 
    end
