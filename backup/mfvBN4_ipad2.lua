  ...
  for i=1,#(as) do
    ...
    if as[i].drag==true or as[i].starget==1 then
      ...
      -- if xi~=-1 and yi~=-1 then
        if as[i].trow==-1 and as[i].tcol==-1 then             -- drag the aiming scope
          dragDrop(ehl[2][3].x, ehl[2][3].y, ehl[xi][yi].x, ehl[xi][yi].y, 5)      -- indirect aim scope drag
          dragDrop(ehl[1][3].x, ehl[1][3].y, ehl[xi][yi].x, ehl[xi][yi].y, 5)      -- indirect aim scope drag
        else
          click( ehl[xi][yi].x, ehl[xi][yi].y )
        end
      end
    end
    ...
    if i<#(as) then
      ...
      if v==false then
      for i=1,3 do
          click(98,985)             -- touch pullback button 2 times
          mSleep(200)
        end
        mSleep(500)
        touchScreen(483,400)          -- ok
        return i,false,bigonefound
      else
...
function gotoBigfootHome()
  ...
  -- mSleep(500)
  -- click(229,469)             -- bigfoot's homeland
  -- mSleep(1000)
