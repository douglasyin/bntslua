...
function proceedAttackSequence( as, battlescene )
  ...
  for i=1,#(as) do
    resetIDLETimer()
    if i==1 then
      local v = waitPixel(BS_abort.x, BS_abort.y, BS_abort.color,35)       --abortx, aborty, abortcolor,
    ...
    if i<#(as) then             -- we are in middle of attack sequence
      ...
      if battlescene=="PVP" then
        rx,ry,rx1,ry1 = waitForTwoColor(BM_stars[1].color,80,BM_stars[1].x,BM_stars[1].y,       -- 
                                      BS_abort.color, 80,BS_abort.x,BS_abort.y,60)
      else
        rx,ry,rx1,ry1 = waitForTwoColor(BM_win[1].color,80,BM_win[1].x,BM_win[1].y,       -- 
                                      BS_abort.color, 80,BS_abort.x,BS_abort.y,60)
      end        
      ...
      if battlescene=="PVP" then
        v = waitPixel(BM_stars[1].x, BM_stars[1].y, BM_stars[1].color,60)
      else
        v = waitPixel(BM_win[1].x, BM_win[1].y, BM_win[1].color,60)
      end
